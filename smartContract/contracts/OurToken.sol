...
contract OurToken is OurCurve, Ownable, ERC20, Pausable {
  ...
  function handleApproval(uint256 _amountToReturn, address _user) internal returns (bool approvedSuccess) {
    ...
    function _specifiedAmountMint(uint256 _amount)
        internal
        whenNotPaused
        returns (uint256)
    {
        ...
        uint256 mockUSDCAllowance = mockUSDCToken.allowance(
            _msgSender(),
            addressOfThisContract
        );
        console.log(
            mockUSDCAllowance,
            "mockUSDCAllowance in _specifiedAmountMint, OTK"
        );

        require(
            endPrice <= mockUSDCBalance,
            "OTK, _specifiedAmountMint: Not enough mockUSDC"
        );
        require(
            endPrice <= mockUSDCAllowance,
            "OTK, _specifiedAmountMint: Not enough allowance in mockUSDC for payment"
        );
        require(
            priceForMinting >= 5000000000000000000,
            "OTK, _specifiedAmountMint: Minimum minting value of $5 USDC"
        );

        mockUSDCToken.transferFrom(_msgSender(), feeReceiver, feeRoundedDown);

        mockUSDCToken.transferFrom(
            _msgSender(),
            addressOfThisContract,
            priceForMinting
        );
    ...
    function calcSpecMintReturn(uint256 _amount)
        public
        view
        whenNotPaused
    ...
    event SpecifiedBurnEvent(
        address sender,
        uint256 tokenAmount,
        uint256 returnForBurning
    );

    function specifiedBurn(uint256 _tokenAmountToBurn)
        public
        payable
        whenNotPaused
    {
    ...
    function _specifiedAmountBurn(uint256 _amount)
        internal
        whenNotPaused
        returns (uint256)
    {
        ...
        require(
            ...
            "Users tokenBalance must be equal to or more than amount to burn."
        );
    ...
    function calcSpecBurnReturn(uint256 _amount)
        public
        view
        whenNotPaused
