...
contract BNJICurve is Ownable, Pausable {
    ...
    function calcPriceForTokenMint(uint256 supply, uint256 tokensToMint)
        public
        view
        returns (uint256)
    {
        ...
        uint256 supplySquared = supply * supply; // line 120

        uint256 supplyAfterMint = supply + tokensToMint; // line 121
        uint256 supplyAfterMintSquared = supplyAfterMint * supplyAfterMint; // line 122

        uint256 step1 = supplyAfterMintSquared - supplySquared; // squareDiff line 123

        uint256 step2 = step1 * USDCscale; // scaledSquareDiff line 124

        uint256 totalPriceForTokensMintingNowInUSDC6digits = step2 /
            curveFactor; // line 125

        uint256 takeOffFactor = 10000;

        uint256 rest = totalPriceForTokensMintingNowInUSDC6digits %
            takeOffFactor;

        uint256 mintResultWithCentsroundedDown = totalPriceForTokensMintingNowInUSDC6digits -
                rest;
    ...
    function calcReturnForTokenBurn(uint256 supply, uint256 tokensToBurn)
        public
        view
        returns (uint256)
    {
        ...
        require(
            supply > 0 && tokensToBurn > 0 && supply >= tokensToBurn,
            "BNJICurve: Sending args must be larger than 0"
        );
        ...
        uint256 burnResultWithCentsroundedDown = returnForTokenBurnInUSDC6digits -
                rest;
